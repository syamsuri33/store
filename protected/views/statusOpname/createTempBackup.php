...
if (isset($_GET['Masterbarang']['MasterBarang_ID'])) {
	if(empty($gdng_ID)){
		...
		if(!empty($_GET['Masterbarang']['MasterBarang_ID'])){
...
<?php 
	...
	if(!empty($inputBrg)){
		foreach ($inputBrg as $result){
			echo '<tr><td>	<input type="text" name="Rak" value="'.$result->Rak.'" /></td>';
			echo '<td>		<input type="text" name="Jumlah" value="'.$result->Jumlah.'"/></td></tr>';
		}
	}
	echo '</table';
?>
